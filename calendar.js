(function(){
  ...
  function month(){const y=state.calCursor.getFullYear(),m=state.calCursor.getMonth(),f=new Date(y,m,1),s=new Date(f);s.setDate(1-((f.getDay()+6)%7));return monthlyBand()+'<div class="month-weekdays">'+["M","T","W","T","F","S","S"].map(x=>'<span>'+x+'</span>').join("")+'</div><div class="month-grid">'+Array.from({length:42},(_,n)=>{const d=new Date(s.getTime()+n*DAY),ev=eventsOn(d),span=ev.find(e=>!e._system&&e.endDate&&e.endDate!==e.date),segment=span?dk(d)===span.date?"start":dk(d)===span.endDate?"end":"middle":"";return '<button class="'+(d.getMonth()!==m?"outside ":"")+(dk(d)===dk(new Date())?"today-cell":"")+'" data-cal-day="'+dk(d)+'"><strong>'+d.getDate()+'</strong>'+(span?'<em class="month-span '+segment+'" style="--event:'+entryColor(span)+'">'+(segment==="start"?escapeHtml(span.title):"&nbsp;")+'</em>':'')+'<span>'+ev.filter(e=>e!==span).slice(0,4).map(e=>'<i style="--event:'+entryColor(e)+';opacity:'+(e._system&&itemComplete(e,d)?".35":"1")+'"></i>').join("")+'</span></button>'}).join("")+'</div>'}
