(function(){
  ...
  function initTimeWheel(prefix){const hour=$("#"+prefix+"Hour"),minute=$("#"+prefix+"Minute");if(!hour||hour.options.length)return;hour.innerHTML=Array.from({length:12},(_,n)=>'<option value="'+(n+1)+'">'+(n+1)+'</option>').join("");minute.innerHTML=Array.from({length:12},(_,n)=>'<option value="'+String(n*5).padStart(2,"0")+'">'+String(n*5).padStart(2,"0")+'</option>').join("")}
  function setTimeWheel(prefix,value="12:00"){initTimeWheel(prefix);let[h,m]=String(value||"12:00").split(":").map(Number),rounded=Math.round((m||0)/5)*5;if(rounded===60){rounded=0;h=(h+1)%24}$("#"+prefix+"Hour").value=String((h%12)||12);$("#"+prefix+"Minute").value=String(rounded).padStart(2,"0");$("#"+prefix+"Period").value=h>=12?"PM":"AM"}
  ...
  function eventsOn(d){
    ...
    const events=state.calOverlays.events?state.planner.events.filter(e=>{const annual=(e.calendarId==="birthdays"||e.recurrence==="yearly")&&e.date?.slice(5)===monthDay,multi=!annual&&e.endDate&&key>=e.date&&key<=e.endDate;return(e.date===key||annual||multi)&&c(e.calendarId).visible!==false}).map(e=>(e.date===key?e:{...e,_displayDate:key})).concat(builtin):[];
  ...
  function renderCalendar(){
    ...
    $$("[data-cal-day]").forEach(b=>b.onclick=()=>{state.calCursor=new Date(b.dataset.calDay+"T12:00");state.calView="day";localStorage.setItem("opalday-cal-view","day");render();window.scrollTo({top:0,behavior:"smooth"})});
  ...
  function dayInterval(e,key){const first=e.date===key,last=!e.endDate||e.endDate===key;let start=first?clockMinutes(e.time):0,end;if(!last)end=1440;else if(e.end)end=clockMinutes(e.end);else end=start+60;if(first&&last&&end<=start)end+=1440;return{start:Math.max(0,Math.min(1440,start)),end:Math.max(0,Math.min(1440,Math.max(start+5,end)))}}
  ...
  function durationBlock(layout,startHour,hourHeight){const e=layout.event,color=entryColor(e),label=escapeHtml(c(e.calendarId).name),left=layout.column/layout.columns*100,width=100/layout.columns,top=(layout.start-startHour*60)/60*hourHeight,height=Math.max(38,(layout.end-layout.start)/60*hourHeight),attr=e.source==="builtin"?"":' data-event="'+e.id+'"',startLabel=layout.start===0&&e.date!==dk(state.calCursor)?"Continues":minutesLabel(layout.start),endLabel=minutesLabel(layout.end);return'<button class="duration-event"'+attr+' style="--event:'+color+';top:'+top+'px;height:'+height+'px;left:calc('+left+'% + 3px);width:calc('+width+'% - 6px)"><strong>'+escapeHtml(e.title)+'</strong><small>'+startLabel+'–'+endLabel+' · '+label+'</small></button>'}
  function day(){
    ...
    const key=dk(state.calCursor),layouts=layOutDayEvents(timed,key),earliest=layouts.length?Math.min(...layouts.map(x=>x.start)):360,latest=layouts.length?Math.max(...layouts.map(x=>x.end)):1440,startHour=Math.min(6,Math.floor(earliest/60)),endHour=Math.max(24,Math.min(24,Math.ceil(latest/60))),hourHeight=76,hours=Array.from({length:endHour-startHour+1},(_,n)=>startHour+n);return top+'<div class="duration-day" style="height:'+((endHour-startHour)*hourHeight)+'px;--hour-height:'+hourHeight+'px"><div class="duration-hours">'+hours.map((h,n)=>'<div style="top:'+(n*hourHeight)+'px"><time>'+new Date(2000,0,1,h%24).toLocaleTimeString([],{hour:"numeric"})+'</time></div>').join("")+'</div><div class="duration-event-layer">'+layouts.map(x=>durationBlock(x,startHour,hourHeight)).join("")+'</div></div>'
  }
  ...
  function openEvent(id){
    ...
    $("#eventModalTitle").textContent=e?"Edit event":"Add event";$("#eventTitle").value=e?.title||"";$("#eventDate").value=e?.date||dk(state.calCursor);$("#eventEndDate").value=e?.endDate||e?.date||dk(state.calCursor);$("#eventAllDay").checked=e?.allDay??!e?.time;$("#eventTime").value=e?.time||"";$("#eventEnd").value=e?.end||"";$("#eventNotify").checked=!!e?.notification?.enabled;setTimeWheel("eventNotify",e?.notification?.time||legacyReminderTime(e));$("#eventNotifyScope").value=e?.notification?.scope||"every";$("#eventNotifyDays").value=e?.notification?.days||"start";$("#eventCalendar").innerHTML=options();$("#eventCalendar").value=e?.calendarId||"mine";$("#deleteEvent").classList.toggle("hidden",!e);toggleEventTimes();openModal("#eventModal");
  }
  ...
  $("#saveEvent").onclick=()=>{const title=$("#eventTitle").value.trim(),start=$("#eventDate").value,end=$("#eventEndDate").value||start;if(!title)return toast("Name the event");if(end<start)return toast("End date must follow start date");let e=state.editEvent?state.planner.events.find(x=>x.id===state.editEvent):null;if(!e){e={id:uid(),createdAt:new Date().toISOString(),source:"manual"};state.planner.events.push(e)}e.title=title;e.date=start;e.endDate=end;e.allDay=$("#eventAllDay").checked;e.time=e.allDay?null:$("#eventTime").value||null;e.end=e.allDay?null:$("#eventEnd").value||null;e.calendarId=$("#eventCalendar").value;e.recurrence=e.calendarId==="birthdays"?"yearly":null;const scope=$("#eventNotifyScope").value;e.notification={enabled:$("#eventNotify").checked,time:readTimeWheel("eventNotify"),leadMinutes:null,days:$("#eventNotifyDays").value,scope,occurrenceDate:scope==="once"?(state.view==="today"?dk(new Date()):dk(state.calCursor)):null};if(state.editEvent)e.userEdited=true;state.editEvent=null;closeModals();calendarChanged();toast(e.recurrence==="yearly"?"Birthday saved every year":end>start?"Multi-day event saved":"Event saved")};
