(function(){
  ...
  function eventsOn(d){
    ...
    const systems=state.planner.items.filter(i=>systemOn(i,d)).map(i=>Object.assign({},i,{_system:true,time:i.fixedTime}));
  ...
  function chip(e){const color=entryColor(e),attr=e.source==="builtin"?"":e._system?' data-system="'+e.id+'"':' data-event="'+e.id+'"',label=e._system?(e.kind==="medication"?(medState(e,state.calCursor)==="overdue"?"OVERDUE · Hard deadline":"Hard medication deadline"):cadenceLabel(e)):escapeHtml(c(e.calendarId).name);return '<button class="event-chip kind-'+(e.kind||"event")+'"'+attr+' style="--event:'+color+'"><strong>'+escapeHtml(e.title)+'</strong><small>'+fmt(e.time)+' · '+label+'</small></button>'}
