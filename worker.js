...
function buildWidgetToday(planner, now, timeZone) {
  ...
  const reminderEventRows = reminders.filter(item => !item.time).map(item => ({
    id: item.id, kind: "reminder", title: item.title,
    startTimeLabel: "Anytime", endTimeLabel: null, timeLabel: "Anytime", status: item.status,
  ...
  const whatsHappening = medicationEventRows.concat(reminderEventRows, events.filter(event => event.allDay).map(event => ({
